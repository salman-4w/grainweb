  ...
  module ByPassword
    ...
    def self.included(recipient)
      ...
      recipient.class_eval do
        ...
        # FIXME: chnange validation back
        # validates_length_of       :new_password, :within => 6..40, :if => :password_required?
        validates_length_of       :new_password, :within => 3..40, :if => :password_required?
        # TODO: implement me
    ...
    module ModelInstanceMethods
      ...
      def encrypt_password
        raise Intersys::MethodNotImplemented
        # return if password.blank?
        # self.salt = self.class.make_token if new_record?
        # self.crypted_password = encrypt(password)
      end
      def password_required?
        password.blank? || !new_password.blank?
      end
